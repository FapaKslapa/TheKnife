...
public class RecensioneController {
    ...
    /** Card per la modifica di una recensione */
    @FXML private VBox modificaRecensioneCard;
    @FXML private TextField modIdRistoranteField, modIdUtenteField, modTitoloField;
    ...
    private void mostraFormModifica(Recensione recensione) {
        ...
        modIdRistoranteField.setText(recensione.getKey_r());
        modIdUtenteField.setText(recensione.getKey_user());
    ...
    private void setupModStelleBox() {
        ...
        for (int i = 1; i <= 5; i++) {
            ...
            stella.setOnMouseEntered(e -> {
                Polygon s = (Polygon) e.getSource();
                int val = (int) s.getUserData();
                for (int j = 0; j < modStelleBox.getChildren().size(); j++) {
                    Polygon st = (Polygon) modStelleBox.getChildren().get(j);
                    st.setFill(j < val ? stellaColorHover : stellaColorVuota);
                }
            });
            stella.setOnMouseExited(e -> {
                for (int j = 0; j < modStelleBox.getChildren().size(); j++) {
                    Polygon st = (Polygon) modStelleBox.getChildren().get(j);
                    st.setFill(j < votoModifica ? stellaColorPiena : stellaColorVuota);
                }
            });
            stella.setOnMouseClicked(e -> {
                Polygon s = (Polygon) e.getSource();
